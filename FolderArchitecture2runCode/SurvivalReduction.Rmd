...
In this document we describe the required analysis to estimate a baseline survival and the post-oil spill survival for common bottlenose dolphins in Barataria Bay, which combined provide a survival reduction.
...
The way we quantify that reduction in survival is by comparing a population average baseline survival derived from the Siler model results previously reported by Schwacke et al. (2017) with a post oil spill population average survival obtained in a separate dedicated spatially explicit capture recapture (SECR) model for photo-identification data collected during 2010-2019 surveys in BB (Glennie et al. in prep). 

However, these two sources of information on survival reflect estimates for different components of the population. In particular, the SECR study reflects only a population average survival that is not age- or sex- specific and that applies only to the fraction of the population that can be resighted - i.e., animals that have distinctive markings to be uniquely identified. 

Since animals are identified based on natural markings on dorsal fins, and that young animals do not have such markings, younger animals are less likely to be considered in the SECR study. 

Further, since we know from the Siler model that survival is sex and age dependent, this means that the population average survival based on the SECR results alone would be biased, closer to a survival for older animals and ignoring the younger animal survival. 

Here he describe in detail the post-processing required to transform the sex- and ag-e specific baseline survival estimates from the Siler model to a population average survival compared to that of Glennie et al. 2021.
...
We consider a dataset containing known-age animals (from health assessments, etc.) from Barataria Bay (Lori Schwacke, unpublished data), and performed a binary logistic regression to estimate the probability of an animal being marked as a function of age and other covariates. Data were analyzed from 54 dolphins in BB ranging in age from 2.5 to 42 years with recorded presence or absence of distinctive marks (such as would be used in photo-ID surveys). For each of these animals there was a record of whether the animals had markings that would allow them to be marked by photo-ID, and hence being included in the pool of animals used to estimate survival from the SECR model.
...
The mean of the survival pre oil spill was `r round(mean(meanS$S),3)` (95% credible intervals of `r round(quantile(meanS$S,prob=0.025),3)`;`r round(quantile(meanS$S,prob=0.975),3)`) while the mean of the survival post oil spill was `r round(mean(SpostOilBB$S),3)` (95% credible intervals of `r round(quantile(SpostOilBB$S,prob=0.025),3)`;`r round(quantile(SpostOilBB$S,prob=0.975),3)`), leading to an average reduction in survival post-oil spill of `r round(mean(SpostOilBB$S/meanS$S),3)` (95% credible intervals of `r round(quantile(SpostOilBB$S/meanS$S,prob=0.025),3)`;`r round(quantile(SpostOilBB$S/meanS$S,prob=0.975),3)`). Below we present side by side the comparison of pre and post oil survivals, where the reduction is survival is evident.
