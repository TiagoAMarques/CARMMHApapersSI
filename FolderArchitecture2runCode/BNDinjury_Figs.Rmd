...
plot(x, q, type = "l", col = "springgreen4", lwd=2, xlim=c(0,28), xlab=expression(paste("Density dependence shape parameter ", rho)), ylab="Probability density", las = 1, family="A", cex.axis=1.2,cex.main=1.5,cex.lab=1.1)
...
"dodgerblue"), lty = c(1,5), lwd=2, cex=1.1, inset = 0.05)

par(op)


rhos <- with(SpInfo,(rhoshift+rgamma(1000000, shape = rhoshape, scale = rhoscale)))
text(21,0.1,labels=paste("mean:",round(mean(rhos),2)),  family="A",cex=1.1)
text(21,0.08,labels=paste("median:",round(median(rhos),2)),  family="A",cex=1.1)
text(21,0.06,labels=paste0("95% equal-tailed interval: ",round(quantile(rhos,p=0.025),2),"-",round(quantile(rhos,p=0.975),2)),  family="A",cex=1.1)
```
...
plot(Nts,ft(Nt=Nts,rho=6.4,Nnom=3045,Fmax=0.35,Fnom=0.24),type="n",ylab="",
     xlab=TeX("Population size"),ylim=c(0.18,0.4), las = 1, family="A", cex.axis=1.2,cex.main=1.5,cex.lab=1.1)
for(i in 1:1000){
  lines(Nts,ft(Nt=Nts,rho=rhos2017[i],Nnom=3045,Fmax=0.35,Fnom=0.24),lty=2,lwd=0.5,col="grey")
}
points(Nts,ft(Nt=Nts,rho=6.4,Nnom=3045,Fmax=0.35,Fnom=0.24),type="l",col="springgreen4",lwd=3, las = 1, family="A", cex.axis=1.2,cex.main=1.5,cex.lab=1.1)
...
points(Nts,ft(Nt=Nts,rho=6,Nnom=3045,Fmax=0.35,Fnom=0.24),type="l",col="dodgerblue",lwd=3,lty=5, las = 1, family="A", cex.axis=1.2,cex.main=1.5,cex.lab=1.1)
abline(h=c(0.24,0.35),v=3045,lty=2)

mtext(TeX("Fecundity"),side = 2,line=3,at = 0.5, family="A", cex=1.2)
```
...
col="dodgerblue", ylab = "Probability density", xlab = "Probability of recovery", las = 1, family="A", cex.axis=1.2,cex.main=1.5,cex.lab=1.1) 
set.seed(123)
ry1 <- (l1+rbeta(1000000, a1, b1)*u1)-l1
text(0.45,3,labels=paste("mean:",round(mean(ry1),3)),cex=1.1, family="A")
text(0.45,2.7,labels=paste("median:",round(median(ry1),3)),cex=1.1, family="A")
text(0.45,2.4,labels=paste0("95% equal-tailed interval: ",round(quantile(ry1,p=0.025),3),"-",round(quantile(ry1,p=0.975),3)),cex=1.1, family="A")
```
...
annotate("segment",x=MPD-1,xend=MPD-1,yend=data4$y,y=data4$yend,colour="black",linetype = "dashed", lwd=2, arrow = arrow(type="closed", length = unit(0.3, "inches")))+
 ...
 #   annotate("segment", yend = 1000, y = 4500, x = YTR, xend = YTR,colour = "black", arrow = arrow(type="closed", length = unit(0.1, "inches")),linetype="dashed") +
  geom_vline(xintercept=YTR-1,linetype="dashed",lwd=2)+
  labs(colour="")+
  scale_color_manual(values=c("darkblue","brown3"))+
 ...
 geom_pointrange(data=dattat1, aes(x=0,
               y=Median,ymin=quantile025,ymax=quantile975),size=1.5)+ 
        scale_color_manual(values=c("blue","red"),labels=c('no spill', 'spill'))+
...
The mean population size in 2019, estimated by SCR, was `r round(mean(bas2019$x))` animals (95% percentile CI `r round(quantile(bas2019$x, probs=0.025))`,`r round(quantile(bas2019$x, probs=0.975))`). In comparison, that value given our model would be `r round(mean(colSums(simres[,9,,1])))` (95% percentile CI `r round(quantile(colSums(simres[,9,,1]), probs=0.025))`,`r round(quantile(colSums(simres[,9,,1]), probs=0.975))`).

# Acknowledgments

We thank Carolina Marques for the code and data wrangling that made the last plot much better.
