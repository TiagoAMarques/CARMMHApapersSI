...
For a few of the injury metrics and parameters combinations the fit is poor, in particular for YTR, but since this happens mostly when the elasticity is extremely small the effects are negligible. These problems originate because we are approximating change in a discrete injury metric (YTR) with a continuous function.
...
Here we calculate the injury metrics percent change for a change in 1% on each of the 11 parameters, with that change being evaluated around the mean value of the input parameter.
