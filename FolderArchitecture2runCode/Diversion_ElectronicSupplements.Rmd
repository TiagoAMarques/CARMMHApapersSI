...
Note that the code could take a considerable amount of time to run. The consequence of running the code will be to populate the folder `InOutBySp\Bottlenose_dolphin_BB` with the R workspaces holding the results that the rest of the code in [MS_RiverDiversion_v5.Rmd](MS_RiverDiversion_v5.Rmd) uses to produce the letter to the MMC that forms the basis of the paper.
