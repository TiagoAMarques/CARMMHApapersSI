...
for (i in 1:nrow(elasticity)){
  ...
  if(elasticity$parameter[i] == "ascS") label <- expression(paste(S[f,j]," & ",S[m,j]))
...
Insight into these elasticity values can be obtained by representing the population trajectories considering 0.5 % under and above the mean value for a statistic. Here we illustrate that with the two parameters that were not scalars.
...
plot(colSums(sr[, , 1, 2]), type = "n", ylim = ylims, xlab = "Years post spill", ylab = "Predicted population size", las = 1,main="P(marked)")
# baseline less 0.5% PM
lines(colSums(sr[, , 1, 2]), type = "l", lwd = 0.7, col = 2)
# baseline plus 0.5% PM
lines(colSums(sr[, , 3, 2]), type = "l", lwd = 1, col = 3)
# under oil less 0.5 %
lines(colSums(sr[, , 1, 1]), type = "l", lwd = 0.7, col = 2)
# under oil plus 0.5 %
lines(colSums(sr[, , 3, 1]), type = "l", lwd = 1, col = 3)
# add a legend
legend("bottomright",inset=0.05,cex=1.2,col=2:3,lty=1,legend=c("- 0.5 %","+ 0.5 %"))
...
plot(colSums(sr[, , 1, 2]), type = "n", ylim = ylims, xlab = "Years post spill", ylab = "Predicted population size", las = 1,main="Sm+Sf")
# baseline less 0.5% PM
lines(colSums(sr[, , 1, 2]), type = "l", lwd = 0.7, col = 2)
# baseline plus 0.5% PM
lines(colSums(sr[, , 3, 2]), type = "l", lwd = 1, col = 3)
