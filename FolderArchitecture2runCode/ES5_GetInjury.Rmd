...
library(knitr)
knitr::opts_chunk$set(echo = FALSE, dpi=50, dev='jpeg')
...
Here we consider standard linear models to explain the injury metrics as a function of the input parameters.
...
summary(lmLCY3)
plot(sumres$LCY, predict(lmLCY3), las = 1, cex.axis=1.2,cex.main=1.5,cex.lab=1.1,xlab="Observed Lost Cetacean Years",ylab="Predicted  Lost Cetacean Years")
```
...
Using the most parsimonious model, the proportion of the variability explained by each input retained in the regression model and the remaining unexplained variation can be obtained from the ANOVA table of the linear regression. The proportion of variance explained in Lost Cetacean Years by the initial population size is `r propvarLCY[2]` %, while the proportion exposed accounts for `r propvarLCY[1]` %. Only `r propvarLCY[3]` of the variability remains unexplained.


### YTR

For YTR the following model describes which input parameters help explain the YTR
...
We removing initial population size and survival reduction to obtain a more parsimonious model.

```{r}
lmYTR <- lm(YTR ~ Pexp + inputs$gd, data = sumres)
summary(lmYTR)
plot(sumres$YTR, predict(lmYTR), las = 1, cex.axis=1.2,cex.main=1.5,cex.lab=1.1,xlab="Observed Years to Recovery",ylab="Predicted  Years to Recovery")
```
...
The following model describes which input parameters help explain the MPD

```{r}
lmMPD <- lm(MPD ~ Pexp + Nstart + inputs$gd + inputs$sr, data = sumres)
summary(lmMPD)
#plot(sumres$MPD, predict(lmMPD), las = 1, cex.axis=1.2,cex.main=1.5,cex.lab=1.1)
```

Removing initial population size just to check if the model is more parsimonious, which it happens to be.

```{r}
lmMPD <- lm(MPD ~ Pexp + inputs$gd + inputs$sr, data = sumres)
summary(lmMPD)
plot(sumres$MPD, predict(lmMPD), las = 1, cex.axis=1.2,cex.main=1.5,cex.lab=1.1,xlab="Observed Maximum Proportional Decrease",ylab="Predicted Maximum Proportional Decrease")
```
