---
title: "Comparing model results with existing surveys"
author: ""
date:  \today
output: 
  html_document:
...
Here we provide a comparison with independent abundance estimates for the different taxonomic units.
...
The survey data are taken out of Garrison et al. (2020, 2021), and were compiled in a .xlsx document also provided under the "InputFiles" folder.

```{r}
GOMMAPS <- read_excel("InputFiles/GOMMAPS_ABUNDANCE_ESTIMATES_forTiago.xlsx")
```

Some surveys are for a pair of years, and we recode these for visual display purposes as a single year value, the mid point between those successive years.
...
Here we define the rows corresponding to each taxonomic unit via their corresponding 4 letter code, since the original data file did not include the taxonomic unit 4 letter code we consider in general on other relevant material for our paper.
