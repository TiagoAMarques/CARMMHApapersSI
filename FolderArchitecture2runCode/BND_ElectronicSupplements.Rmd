...
* [BND_InitialPopulation](BND_InitialPopulation.html) – Initial population size distribution and baseline survival. These are taken as data inputs, and the deatil analysis that describes how these were obtained is part of a separate dedicated effort where a spatially explicit capture recapture model was implemented (Glennie et al.,2021).
...
* [AgeOf1stReproduction](AgeOf1stReproduction.html) - Supplementary material with details about how we obtained the age at first reproduction for *Tursiops truncatus*;

* [BNDinjury_Figs](BNDinjury_Figs.html) - obtaining the injury measures presented on the paper;
...
* `InputFiles`: this folder includes a number of files that are treated as data, inputs required to run the code, including the key file `SpeciesDefinitionFile4offshore.xlsx` that contains parameters for all the different taxonomic units in CARMMHA project, including the Barataria Bay bottlenose dolphins.
