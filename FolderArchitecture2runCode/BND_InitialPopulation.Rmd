...
The mean initial population size considering `r length(Nstart)` available samples from the posterior of the SECR model ranged between `r min(Nstart)` and `r max(Nstart)`. The mean population size was `r round(mean(Nstart))` and the median `r round(median(Nstart))` with a 95% credible interval of (`r round(quantile(Nstart,0.025))` ,`r  round(quantile(Nstart,0.025))`). We can plot the distribution of the abundance estimates (removing estimates above double the mean for better visualization)
...
The population average survival considering `r length(SpostOilBB)` available samples from the posterior of the SECR model ranged between `r round(min(SpostOilBB),3)` and `r round(max(SpostOilBB),3)`. The mean survival was `r round(mean(SpostOilBB),3)` and the median `r round(median(SpostOilBB),3)` with a 95% credible interval of (`r round(quantile(SpostOilBB,0.025),3)` ,`r  round(quantile(SpostOilBB,0.975),3)`). We can plot the distribution of the survival post oil spill estimates 
```{r}
