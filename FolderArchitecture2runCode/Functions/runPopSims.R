runPopSims <- function(Sp, nsims, nyears, type = "Sim", parS = NULL,
  ...
  pars2save <- data.frame(
    sim = 1:nsims, ASM = NA, ASMTtru = NA, N0 = NA, pe = NA, per = NA, Fmax = NA,
    Fnom = NA, FnomTtru = NA, rho = NA, br = NA, por = NA, spos = NA, ascS = NA, SR = NA, BrTt = NA,
    PorTt = NA, y2R = NA, srTtru = NA, fecRed = NA, PM = NA, paPM = NA, meanS = NA, meanSiler = NA, meanSilerF = NA,
  ...
  # forcing random draws (as there was an inherent non-random order in the draws)
  ...
  if (type != "Sim") {
    ...
    if (parS != "ascS") {
      # if doing sensitivity analysis just use the mean survival
      # Ttru
      pxFs <- matrix(rep(rowMeans(pxFs), ncol(pxFs)), nrow(pxFs), ncol(pxFs))
      pxMs <- matrix(rep(rowMeans(pxMs), ncol(pxMs)), nrow(pxMs), ncol(pxMs))
      # Sp
      SpxFs <- matrix(rep(rowMeans(SpxFs), ncol(SpxFs)), nrow(SpxFs), ncol(SpxFs))
  ...
  # keeping correlation structure between N and post-spill survival
  # only relevant in Ttru, inconsequent everywhere else
  SpostOilBB <- SpostOilBB[index.NOT.outlier]
  ...
  if (type != "Sim") {
    ...
    if (parS != "N0") {
      # if doing sensitivity analysis just use the mean
      meanN0 <- mean(Nstart)
      Nstart <- rep(meanN0, length(Nstart))
    }
  }
  # proportion exposed
  if (type != "Sim") {
  ...
  # need to adjust for scaling for non Ttru
  a1stRsims <- a1stRsimsTtru / scaling
  if (type != "Sim") {
    # if running a sensitivity analysis
    if (parS != "ASM") {
      # if doing sensitivity analysis for other parameter not ASM just repeat the mean ASM nsims times
      meanASM <- with(Ttru, sha1str * sca1str) / scaling
      meana1stRTtru <- with(Ttru, sha1str * sca1str)
      a1stRsims <- rep(meanASM, nsims) / scaling
      a1stRsimsTtru <- rep(meanASM, nsims)
    }
  }
  # the modeling requires an integer age
  a1stRsims <- round(a1stRsims)
  a1stRsimsTtru <- round(a1stRsimsTtru)
  pars2save$ASM <- a1stRsims
  pars2save$ASMTtru <- a1stRsimsTtru
  #------------------------------------------------------------------------------
  ...
  simres <- array(
    data = NA, dim = c(dimm, nyears, nsims, 2),
    dimnames = list(
      class = paste0("c", 1:dimm),
  ...
  for (i in 1:nsims) {
    ...
    # Note this object is only used way below but it's here to stay close to M0BND
    M0 <- getM(
      na = na, dimm = ns * nc * na, femalesur = femalesur, malesur = malesur,
    ...
    # note dimm = number of sexes * number of conditions * number of age classes
    M0BND <- getM(
      na = naBND, dimm = ns * nc * naBND, femalesur = pxFs[, iS], malesur = pxMs[, iS],
    ...
    # population baseline pre-spill average survival consistent with SCR study
    meanS <- sum(pxMF[, iS] * qijs)
    # saves population average baseline survival
    pars2save$meanS[i] <- meanS
    pars2save$dnom[i] <- sum((distNominal[1:61] + distNominal[62:122])^2)
    pars2save$pred[i] <- sum(predictions[iPM, ]^2)
    pars2save$qijs[i] <- sum(qijs^2)
    pars2save$pxMF[i] <- sum(pxMF[, iS])
    pars2save$M0BND[i] <- sum(M0BND)
    pars2save$meanSiler[i] <- sum(pxMF[, iS] * distNominal)
    pars2save$meanSilerF[i] <- sum(pxFs[, iS] * distNominal[1:61]/sum(distNominal[1:61]))
    pars2save$meanSilerM[i] <- sum(pxMs[, iS] * distNominal[62:122]/sum(distNominal[62:122]))
    #population averaged Proportion Marked
    ...
    for (j in 1:(nyears - 1)) {
      ...
      # oil spill scenario
      Moil <- getM(
        na = na, dimm = ns * nc * na, femalesur = femalesur * LowSalSR_Yr,
      ...
      if (Sp %in% BB_BND_Sp) {
        ...
        Mnooil <- getM(
          na = na, dimm = ns * nc * na, femalesur = femalesur, malesur = malesur, srf = 1,
