runSimsSensitivity <- function(Sp,nsims,nyears,parS){
  # This function runs a simulation, for 
  # a given species, a number of iterations and for nyears
  # under sensitivity mode - i.e. all parameters at the mean nominal values
  # except for parameter parS that uses the usual distribution as in normal mode
  # Inputs
  ...
  # if parS is a1R do business as usual else use the mean of the gamma distribution
  if(parS=="a1R"){a1stRsims <- with(Ttru, rgamma(nsims, shape=sha1str, scale=sca1str))}else{
    a1stRsims <- rep(with(Ttru,sha1str*sca1str),nsims)  
  }
  ...
  for(i in 1:nsims) {
    ...
    # if parS is N0 do business as usual else use mean of iterations for N0
    if(parS=="N0"){
    ...
    # if parS is pe do business as usual else use mean of iterations for pe
    if(parS=="pe"){
    ...
    # if parS is "per" do business as usual else use use the mean of the simulated values
    if(parS=="per"){pexprecsim  <- pexprecsims[i]}else{
      pexprecsim  <- mean(pexprecsims)  
    }
    #------------------------------------------------------------
    ...
    # Fecundity
    # get DD fecundity parameters
    # if parS is Fmax do business as usual else use the mean of the simulated values
    if(parS=="Fmax"){Fmaxsim <- Fmaxsims[i]}else{
      Fmaxsim <- mean(Fmaxsims)
    }
    # if parS is Fnom do business as usual else use the mean of the simulated values
    if(parS=="Fnom"){Fnomsim <- Fnomsims[i]}else{
      Fnomsim <- mean(Fnomsims)
    }
    # if parS is rho do business as usual else use the mean of the simulated values
    if(parS=="rho"){rhosim  <- rhosims[i]}else{
      rhosim <- mean(rhosims)
    }
    ...
    #get Baseline reproductive success rate
    # if parS is br do business as usual else use the mean of the simulated values
    if(parS=="br"){pRepbasesim <- pRepbasesims[i]}else{
      pRepbasesim <- mean(pRepbasesims)
    }
    #------------------------------------------------------------
    ...
    if(parS=="spos"){Spostspill <- SpostOilBB[iterSCR]}else{Spostspill <- mean(SpostOilBB)}
    ...
    # get post spill reproductive success rate
    # if parS is por do business as usual else use the mean of the simulated values
    if(parS=="por"){pRepPostsim <- pRepPostsims[i]}else{
      pRepPostsim <- mean(pRepPostsims)
    }
    ...
    if (Sp!="Ttru" & Sp!="Schwackeetal2017"){
      # get Baseline reproductive success rate for Ttru
      if(parS=="BrTt"){TtrupRepbasesim <- TtrupRepbasesims[i]}else{
        TtrupRepbasesim <- mean(TtrupRepbasesims)
      }
      
      # get post spill reproductive success rate for Ttru
      if(parS=="PorTt"){TtrupRepPostsim <- TtrupRepPostsims[i]}else{
        TtrupRepPostsim <- mean(TtrupRepPostsims)
      }
  ...
  ft <- "simres"
  # file specifics
  fs <- paste0 (nsims,"Sens",parS)
  # file extension
  fe <- ".RData"
  #a bespoke function to make code shorter when defining file names
  getfilename <- function(a1,a2,a3,a4){return(paste0(a1,a2,a3,a4))}
  save(simres, injury, RunOnThe, fecs, TimeSpent, file=getfilename(fp,ft,fs,fe))  
