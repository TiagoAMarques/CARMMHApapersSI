...
# "BS" - baseline survival  -  uses mean of realizations of survival curves if par not "BS" in runSimsSensitivity
...
#list of parameters
parS <- c("a1r","N0","pe","per","Fmax","Fnom","rho","br","por","spos","ascS","BS","SR","BrTt","PorTt")
...
for (j in parS) {
  if(!(j %in% c("BrTt","PorTt")) | i !="Ttru") runPopSims(Sp = i, type = "Sens", nsims = ns, nyears = ny, par =j)
  }
}