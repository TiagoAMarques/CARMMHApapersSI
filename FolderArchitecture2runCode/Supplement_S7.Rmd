---
title: 'Supplement S7: Model evaluation via sensitivity and elasticity analysis for pelagic taxonomic units'
author: ""
date: \today
output: 
...
* Elasticity analysis. The aim is to estimate the proportional change in the injury metrics caused by a proportional change in each model parameter. This allows us to quantify the "inherent sensitivity" of each model parameter.  Elasticity is traditionally used in the population dynamics literature (Benton & Grant 1999, Caswell 2006) to determine the proportional change in population growth rate to a proportional change in population vital rate; here we extend the concept to cover the injury metrics.
...
* based on the models and results from Roberts et al., with distributions  available via parametric bootstrap resampling; see also [Supplement_S3](Supplement_S3.html) for details.
    - $N_0$: initial population size
    - $p_e$: the proportion of the population exposed to oil

* based on the Expert Elicitation exercise, see SI document [Supplement_S2](Supplement_S2.html), for details
...
* based on earlier published work on bottlenose dolphin data, then scaled for each taxonomic unit by [Supplement_S4](Supplement_S4.html). For additional details regarding the bottlenose dolphin data see also Schwacke et al. 2017 and Schwacke et al. 2021 (SI, Table S2.1 - Input parameters for population model).
    - $S_m$ and $S_f$: a vector of age and sex dependent survival for males and females, based on the Siler model parameters, see [Supplement_S5](Supplement_S5.html) for details
...
From this model we can predict the injury metric given a value $v$ for the input parameter $k$, represented by $\hat I_k^v$. We represent the nominal mean value of the input parameter $k$ as $\bar k$. Therefore the elasticity is computed as

$$E_k(I)=\frac{\hat I_k^{1.005\bar k}-\hat I_k^{0.995\bar k}}{\hat I_k^{\bar k}}$$.
...
Caswell H. 2006 *Matrix Population Models: Construction, Analysis, and Interpretation*, 2nd ed. Sinauer Associates, Inc, Sunderland, Massachusetts. 
